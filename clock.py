...
def timer1():
    global LEDpoints, pathcount, pathparadecount, pathblinkycount
    t = localtime()
    mysec = str0(t.tm_sec)
    mymin = str0(t.tm_min)
    myhour = t.tm_hour
    ...
    LEDlib.createCharColourSolid(canvas1,10, 4, FruitList[myday],LEDpoints)
    # defender animation
    ...
    LEDlib.createCharColourSolid(canvas1,x, y, charGalaxian,LEDpoints)
    x,y = smooth_pathparade[pathblinkycount]
    LEDlib.createCharColourSolid(canvas1,x, y, charPacmanBlinky,LEDpoints)
    pathparadecount = pathparadecount + 1
    pathblinkycount = pathblinkycount + 1
    if pathparadecount >= len(smooth_pathparade): pathparadecount = 0
    if pathblinkycount >= len(smooth_pathparade): pathblinkycount = 0
