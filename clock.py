...
def timer1():
    ...
    LEDpoints = []
    LEDlib.psize = 3# 7*((time()-starttime) % 7)/10+3
    LEDlib.createCharColourSolid(canvas1,400-10*((time()-starttime) % 45), 200, charPacmanBlinky,LEDpoints)
    LEDlib.createCharColourSolid(canvas1,10*((time()-starttime) % 45), 200, charDefender,LEDpoints)
    mymin = t.tm_min
    myhour = t.tm_hour
    ...
    LEDlib.psize = 2
    LEDlib.createCharColourSolid(canvas1,x, 320-y, charSun2,LEDpoints)
